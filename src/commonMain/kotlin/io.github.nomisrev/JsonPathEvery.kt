...
public inline val Every<JsonElement, JsonElement>.`object`:
  Every<JsonElement, Map<String, JsonElement>>
...
public operator fun Every<JsonElement, JsonElement>.get(
  name: String
): Every<JsonElement, JsonElement> = select(name)
...
public fun Every<JsonElement, JsonElement>.filterIndex(
  p: (Int) -> Boolean
): Every<JsonElement, JsonElement> = array compose FilterIndex.list<JsonElement>().filter(p)
